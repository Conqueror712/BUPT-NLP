...
from utils import clear_dir, split_file_dir_name_ext
...
def main():
    # 构建检索器
    retriever = BM25Retriever()  # 或者使用 NaiveBM25Retriever()

    # 构建索引
    dataset = load_dataset()  # 加载数据集，格式为列表，每个元素为一个文档
    retriever.index(dataset)

    # 执行检索
    queries = load_queries()  # 加载查询数据集，格式为列表，每个元素为一个查询字符串
    results = retriever.search(queries)

    # 输出检索结果
    for query_id, doc_ids in results.items():
        print(f"Query ID: {query_id}")
        print(f"Retrieved Documents: {doc_ids}")
        print()

def load_dataset():
    # 加载数据集的实现，可以根据你的数据集格式进行自定义
    # 返回一个文档列表，每个元素为一个文档
    dataset = [
        "Document 1",
        "Document 2",
...
def load_queries():
    # 加载查询数据集的实现，可以根据你的查询数据集格式进行自定义
    # 返回一个查询列表，每个元素为一个查询字符串
    queries = [
        "Query 1",
        "Query 2",
        "Query 3",
